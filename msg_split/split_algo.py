...
def _break_source(opts: Options, pos_end: int, source: str) -> tuple[str, int]:
    header = opts.header + ''.join([b.to_open_tag() for b in opts.open_brackets \
                                    if b.pos_start < opts.cursor_pos])
...
def _process_open_bracket(br: Bracket, opts: Options, source: str) ->  str | None:
    full_len_with_tag = br.paired_len + opts.len_open_before_pos + br.pos_start - opts.cursor_pos
...
def _process_close_bracket(br: Bracket, opts: Options) ->  None:
    ...
    if bracket.pos_start < opts.cursor_pos:
        opts.header = bracket.to_open_tag() + opts.header
    opts.chunk_idx+=1
